...
bool Game::partnerHasNoCardsLeft(int playerTurn){
    return players[(playerTurn + 4 - 2) % 4]->hasStatus(HASNOCARDSLEFT);
}

bool Game::lastThreePlayersPassedOrHaveNoCardsLeft(int playerTurn){
    return ((players[(playerTurn + 4 - 3) % 4]->hasStatus(PASSED) || players[(playerTurn + 4 - 3) % 4]->hasStatus(HASNOCARDSLEFT)) &&
            (players[(playerTurn + 4 - 2) % 4]->hasStatus(PASSED) || players[(playerTurn + 4 - 2) % 4]->hasStatus(HASNOCARDSLEFT)) &&
            (players[(playerTurn + 4 - 1) % 4]->hasStatus(PASSED) || players[(playerTurn + 4 - 1) % 4]->hasStatus(HASNOCARDSLEFT)));
}
